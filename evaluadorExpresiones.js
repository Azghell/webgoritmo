// evaluadorExpresiones.js (Estado Estable: Post Fase 4 original, antes de errores)
...
Webgoritmo.Expresiones.Util = {
    ...
    realizarComparacion: function(a,b,op,nL){
        ...
        else if (op !== '=' && op !== '<>' && op !== '==') throw new Error(`L${nL}: Comparación entre tipos incompatibles (${typeof a} ${op} ${typeof b}).`);
        switch(op){
...
Webgoritmo.Expresiones.convertirInfijoAPostfijo = function(listaTokens) {
    ...
    for (let i = 0; i < listaTokens.length; i++) {
        ...
        if ((token.valor === '-' || token.valor.toUpperCase() === 'NO') && token.tipo === Tipos.OPERADOR) { // NO también es unario
            ...
            if (!prevToken || prevToken.tipo === Tipos.OPERADOR || prevToken.tipo === Tipos.PARENTESIS_IZQ || prevToken.tipo === Tipos.COMA || prevToken.tipo === Tipos.CORCHETE_IZQ) {
                let valorUnario = token.valor === '-' ? '_UMINUS_' : 'NO'; // Usar 'NO' directamente para el operador unario NO
    ...
    for (const token of tokensProcesados) {
        switch (token.tipo) {
            ...
            case Tipos.PARENTESIS_DER:
                ...
                if (pilaOperadores.length === 0 || pilaOperadores[pilaOperadores.length - 1].tipo !== Tipos.PARENTESIS_IZQ) throw new Error("Paréntesis desbalanceados (falta '(').");
            ...
            case Tipos.CORCHETE_DER:
                ...
                if (pilaOperadores.length === 0 || pilaOperadores[pilaOperadores.length - 1].tipo !== Tipos.CORCHETE_IZQ) throw new Error("Corchetes desbalanceados (falta '[').");
                pilaOperadores.pop();
                colaSalida.push({ tipo: Tipos.OPERADOR_ACCESO_ARREGLO, valor: '[]', original: '[]', dimensions: 1 }); // Asume 1D
                break;
            case Tipos.OPERADOR_UNARIO: // Los unarios tienen alta precedencia y se meten directo (o según su precedencia si hubiera otros unarios)
                 while (pilaOperadores.length > 0) {
                    ...
                    if (Util.obtenerPrecedenciaOperador(opEnPila) > Util.obtenerPrecedenciaOperador(token) ) { // Solo si el de la pila es MAYOR (unarios son derecha-asoc)
            ...
            case Tipos.COMA:
                ...
                if (pilaOperadores.length === 0 || (pilaOperadores[pilaOperadores.length-1].tipo !== Tipos.PARENTESIS_IZQ && pilaOperadores[pilaOperadores.length-1].tipo !== Tipos.CORCHETE_IZQ)) {
                    // Comentado para permitir comas en Escribir, se validará en el handler de Escribir o función.
                    // throw new Error("Coma inesperada o paréntesis/corchetes no balanceados.");
                }
                break;
        }
    }
    while (pilaOperadores.length > 0) { const op = pilaOperadores.pop(); if (op.tipo === Tipos.PARENTESIS_IZQ || op.tipo === Tipos.CORCHETE_IZQ) throw new Error("Paréntesis/Corchetes desbalanceados."); colaSalida.push(op); }
    return colaSalida;
};
...
Webgoritmo.Expresiones.evaluarRPN = async function(colaRPN, ambitoActual, numeroLinea) {
    ...
    function obtenerValor(item) { return UtilInterprete.obtenerValorRealVariable(item, ambitoActual, numeroLinea); }
    ...
    for (const token of colaRPN) {
        ...
        switch (token.tipo) {
            ...
            case Tipos.IDENTIFICADOR: pilaValores.push(obtenerValor(token.valor)); break; // Obtener valor de variable
            case Tipos.OPERADOR_ACCESO_ARREGLO:
                ...
                let indiceAcceso = obtenerValorReal(pilaValores.pop(), numeroLinea, "Índice de arreglo");
                let descriptorAcceso = pilaValores.pop(); // Ya debería ser el descriptor del arreglo
                ...
                pilaValores.push(descriptorAcceso.valor[indiceAcceso]);
                break;
            case Tipos.OPERADOR_UNARIO:
                if (pilaValores.length < 1) throw new Error(`L${numeroLinea}: Falta operando para '${token.valor}'.`);
                let operandoUnario = obtenerValorReal(pilaValores.pop(), numeroLinea);
                if (token.valor === '_UMINUS_') { if(typeof operandoUnario !=='number') throw new Error(`L${numeroLinea}: Menos unario solo para números.`); pilaValores.push(-operandoUnario); }
                else if (token.valor === 'NO') { pilaValores.push(UtilExpr.realizarNOT(operandoUnario, numeroLinea)); }
                else throw new Error(`L${numeroLinea}: Operador unario '${token.valor}' desconocido.`);
                break;
            case Tipos.OPERADOR:
                if (pilaValores.length < 2) throw new Error(`L${numeroLinea}: Faltan operandos para '${token.valor}'.`);
                let opDer = obtenerValorReal(pilaValores.pop(), numeroLinea); let opIzq = obtenerValorReal(pilaValores.pop(), numeroLinea);
                switch (token.valor.toUpperCase()) {
                    case '+': pilaValores.push(UtilExpr.realizarSuma(opIzq, opDer, numeroLinea)); break;
                    case '-': pilaValores.push(UtilExpr.realizarResta(opIzq, opDer, numeroLinea)); break;
                    case '*': pilaValores.push(UtilExpr.realizarMultiplicacion(opIzq,opDer,numeroLinea)); break;
                    case '/': pilaValores.push(UtilExpr.realizarDivision(opIzq,opDer,numeroLinea)); break;
                    case 'MOD': case '%': pilaValores.push(UtilExpr.realizarModulo(opIzq,opDer,numeroLinea)); break;
                    case '^': pilaValores.push(UtilExpr.realizarPotencia(opIzq,opDer,numeroLinea)); break;
                    case 'Y': case '&&': pilaValores.push(UtilExpr.realizarAND(opIzq,opDer,numeroLinea)); break;
                    case 'O': case '||': pilaValores.push(UtilExpr.realizarOR(opIzq,opDer,numeroLinea)); break;
                    case '=': case '==': case '<>': case '!=': case '<': case '>': case '<=': case '>=':
                        pilaValores.push(UtilExpr.realizarComparacion(opIzq,opDer,token.valor,numeroLinea)); break;
    ...
    return obtenerValorReal(pilaValores[0], numeroLinea, "Resultado de expresión");
};

Webgoritmo.Expresiones.evaluarExpresion = async function(expresionComoTexto, ambitoActual, numeroLinea = 'expresión') {
    console.log(`[evaluadorExpresiones (ESTADO ESTABLE REVERTIDO)] evaluando: "${expresionComoTexto}"`);
    ...
    try {
        ...
        // console.log(`[Tokens] para "${expresionComoTexto}":`, JSON.stringify(tokens.map(t=>({t:t.tipo,v:String(t.valor)}))));
        const rpn = Webgoritmo.Expresiones.convertirInfijoAPostfijo(tokens);
        // console.log(`[RPN] para "${expresionComoTexto}":`, JSON.stringify(rpn.map(t=>({t:t.tipo,v:String(t.valor)}))));
        const resultado = await Webgoritmo.Expresiones.evaluarRPN(rpn, ambitoActual, numeroLinea);
        // console.log(`[Resultado] para "${expresionComoTexto}":`, resultado);
        return resultado;
    } catch (e) {
...
// Funciones helper copiadas al final del archivo en la versión anterior han sido integradas o eliminadas si eran redundantes.

console.log("evaluadorExpresiones.js (ESTADO ESTABLE REVERTIDO) cargado.");
